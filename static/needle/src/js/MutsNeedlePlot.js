...
var d3_svg_legend = require("./d3-svg-legend");
d3.svg.legend = d3_svg_legend['setLegend'];
require("../css/muts-needle-plot.css");
...
function MutsNeedlePlot (config) {
    ...
    function brushmove() {
        ...
        window.selectedNeedles = [];
        window.categCounts = {};
        for(var key in self.totalCategCounts) {
            if(self.totalCategCounts.hasOwnProperty(key)) {
                categCounts[key] = 0;
            }
        }
...
MutsNeedlePlot.prototype.drawLegend = function(svg) {
    ...
    for(var i = 0, len = orderedDeclaration.length; i < len; i++) {
        var c = orderedDeclaration[i];
    ...
    var showNoShow = function(categ){
        ...
        if (_.indexOf(self.noshow, categ) != -1) {
...
MutsNeedlePlot.prototype.drawRegions = function(svg, regionData) {
    ...
    var getRegionStart = function(region) {
        return parseInt(region.split("-")[0])
    };

    var getRegionEnd = function(region) {
        return parseInt(region.split("-")[1])
    };

    var getColor = this.colorScale;
    ...
    if (typeof regionData == "string") {
        ...
        d3.json(regionData, function(error, regions) {
            ...
            var regionList = formatRegions(regions);
            draw(regionList);
        });
    } else {
        var regionList = formatRegions(regionData);
        draw(regionList);
    }
...
MutsNeedlePlot.prototype.drawNeedles = function(svg, mutationData, regionData) {
    ...
    var stackNeedle = function(pos,value,pointDict) {
      ...
      if (pos in pointDict) {
         ...
         var newHeight = stickHeight + value;
    ...
    function prepareMuts(unformattedMuts) {
        for(var i = 0, len = unformattedMuts.length; i < len; i++) {
            var formatted = formatMutationEntry(unformattedMuts[i]);
...
function repaint(plotConfig) {
    d3.selectAll('svg').remove();
    var targetElement = document.getElementById(plotConfig.targetElement) || plotConfig.targetElement || document.body;
    var currentWidth = $(window).width();
    var windowWidth = plotConfig.preWindowWidth || plotConfig.windowWidth;
    if(plotConfig.width) {
        plotConfig.width = plotConfig.width * (currentWidth / windowWidth);
    } else {
        plotConfig.width = (targetElement.offsetWidth || 1000) * (currentWidth / windowWidth);
    }
    var p = new MutsNeedlePlot(plotConfig);
    plotConfig.preWindowWidth = $(window).width();
}

var Events = require('biojs-events');
Events.mixin(MutsNeedlePlot.prototype);

// module.exports = [MutsNeedlePlot, repaint];
exports.MutsNeedlePlot = MutsNeedlePlot;
